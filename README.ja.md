...
gitwink はオープンソース向けの [SignPath Foundation](https://signpath.org/)
無償コード署名プログラムに参加している — [コード署名ポリシー](CODE_SIGNING_POLICY.md)
を参照。最初の署名済みリリースが出るまで、Windows ビルドは未署名の
ままだ（上記 [ダウンロード](#ダウンロード) を参照）。
